...
def add_nodes_recursive(bi_pairs, rootnode, start_index, end_index):
    ...
    if(start_index > end_index):
        print("Error occured while drawing RNA %d %d" %
...
def setup_coords_recursive(rootnode, parentnode, start_x, start_y, go_x, go_y, NODE_R, PRIMARY_SPACE, PAIR_SPACE, external_multiplier, external_offset):
    ...
    if(len(rootnode.children_) == 1):
        ...
        if(rootnode.children_[0].is_pair_):
            setup_coords_recursive(rootnode.children_[0], rootnode, start_x + go_x * PRIMARY_SPACE, start_y + go_y *
                                   PRIMARY_SPACE, go_x, go_y, NODE_R, PRIMARY_SPACE, PAIR_SPACE, external_multiplier, external_offset)
        elif(rootnode.children_[0].is_pair_ == False and rootnode.children_[0].index_a_ < 0):
            setup_coords_recursive(rootnode.children_[0], rootnode, start_x, start_y, go_x,
                                   go_y, NODE_R, PRIMARY_SPACE, PAIR_SPACE, external_multiplier, external_offset)
        else:
            setup_coords_recursive(rootnode.children_[0], rootnode, start_x + go_x * PRIMARY_SPACE, start_y + go_y *
                                   PRIMARY_SPACE, go_x, go_y, NODE_R, PRIMARY_SPACE, PAIR_SPACE, external_multiplier, external_offset)
        ...
        circle_length = (len(rootnode.children_) + 1) * \
            PRIMARY_SPACE + (npairs + 1) * PAIR_SPACE
        ...
        for ii in range(0, len(rootnode.children_)):
            ...
            if (parentnode == None):
                ...
                rad_angle = length_walker/circle_length * 2 * math.pi / \
                    external_multiplier - math.pi / 2.0 + external_offset
                child_x = rootnode.x_ + \
                ...
                child_go_len = math.sqrt(
                    child_go_x * child_go_x + child_go_y * child_go_y)

                setup_coords_recursive(rootnode.children_[ii], rootnode, child_x, child_y, child_go_x / child_go_len,
                                       child_go_y / child_go_len, NODE_R, PRIMARY_SPACE, PAIR_SPACE, external_multiplier, external_offset)
...
def get_coords_recursive(rootnode, xarray, yarray, PRIMARY_SPACE, PAIR_SPACE):
    ...
    for ii in range(0, len(rootnode.children_)):
        get_coords_recursive(rootnode.children_[
                             ii], xarray, yarray, PRIMARY_SPACE, PAIR_SPACE)
...
class RNARenderer:
    ...
    def setup_tree(self, secstruct, NODE_R, PRIMARY_SPACE, PAIR_SPACE, external_multiplier, external_offset):
        ...
        self.setup_coords(NODE_R, PRIMARY_SPACE, PAIR_SPACE,
                          external_multiplier, external_offset)
    ...
    def get_coords(self, xarray, yarray, PRIMARY_SPACE, PAIR_SPACE):
        ...
        if(self.root_ != None):
            get_coords_recursive(self.root_, xarray, yarray,
                                 PRIMARY_SPACE, PAIR_SPACE)
        else:
            for ii in range(0, len(xarray)):
                xarray[ii] = 0
    ...
    def setup_coords(self, NODE_R, PRIMARY_SPACE, PAIR_SPACE, external_multiplier, external_offset):
        if self.root_ != None:
            setup_coords_recursive(self.root_, None, 0, 0, 0, 1, NODE_R,
                                   PRIMARY_SPACE, PAIR_SPACE, external_multiplier, external_offset)
