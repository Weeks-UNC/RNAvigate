...
class FragMaP(data.Profile):
    ...
    def get_dataframe(
        ...
        FragMaP.calc_zscore(
            self, valid, dataframe, incolumn='Modified_rate_1',
            outcolumn= 'zscore_1', base=['A', 'U', 'C', 'G']
            )
        FragMaP.calc_zscore(
            self, valid, dataframe, incolumn='Modified_rate_2',
            outcolumn= 'zscore_2', base=['A', 'U', 'C', 'G']
            )

        # Frag-MaP profile is the difference in Z-scores
        dataframe.eval(
            'Fragmap_profile = zscore_1 - zscore_2',
            inplace=True
            )
        dataframe.eval(
            'Fragmap_err = sqrt(zscore_1_err ** 2 + zscore_2_err ** 2)',
            inplace=True
            )
    ...
    def calc_zscore(
        ...
        fragmap_err = np.sqrt(
            (dz_dy * sem)**2 + (dz_dx * 0)**2 + (dz_ds * 0)**2
            )
