...
class Transcript(data.Sequence):
    def __init__(
            self, parent, name, sequence, chromosome, strand, coordinates,
            tx_info, cds_coors=None, other_features=None
            ):
    ...
    def get_junctions_annotation(self, **kwargs):
        ...
        junctions = [self.get_tx_range(*sorted(junc))
                     for junc in zip(*junctions)]
        return data.Annotation(
...
class Transcriptome():
    ...
    def get_transcripts(self, transcript_ids):
        ...
        with open(self.annotation) as txome:
            ...
            for entry in txome.readlines():
                ...
                for id in transcript_ids:
                    if f'transcript_id "{id}"' in entry:
                        tx_id = id
                if tx_id is None:
                    continue
        ...
        transcripts = []
        coordinates = [(sorted(start_coors[id]), sorted(stop_coors[id]))
                       for id in transcript_ids]
