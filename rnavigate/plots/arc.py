...
class AP(plots.Plot):
    def __init__(self, num_samples, nt_length, region="all", **kwargs):
        ...
        self.pass_through = ["axis", "seqbar", "title", "profile_panel",
    ...
    def plot_data(self, seq, ct, comp, interactions, interactions2, profile,
                  label, axis=None, seqbar=True, title=True,
        ...
        self.plot_arcs(
            ...
            annotation_gap=annotation_gap)
        self.plot_arcs(
            axis=axis, data=interactions2, panel=interactions2_panel,
            annotation_gap=annotation_gap)
        self.plot_profile(
            axis=axis, profile=profile, annotation_gap=annotation_gap,
            panel=profile_panel, plot_error=plot_error,
            profile_scale_factor=profile_scale_factor)
        for i, annotation in enumerate(annotations):
            self.plot_annotation(axis, annotation=annotation, yvalue=-2-(4*i),
                                 mode=annotation_mode)
        if seqbar:
            self.add_sequence(axis, seq.sequence, yvalue=1-annotation_gap,
                              ytrans="data")
        if title:
            self.add_title(axis, label)
        self.i += 1

    def set_axis(self, axis, annotation_gap=0, xticks=20, xticks_minor=10):
        ...
        axis.spines['bottom'].set(position=('data', -annotation_gap),
                                visible=False)
        axis.spines['top'].set_color('none')
        height = min(300, self.nt_length/2)
        mn, mx = self.region
        axis.set(xlim=(mn - 0.5, mx + 0.5),
               ylim=(-height-1-annotation_gap, height+1),
               xticks=get_ticks(xticks, mn, mx),
               axisbelow=False)
        axis.set_xticks(get_ticks(xticks_minor, mn, mx), minor=True)
    ...
    def add_title(self, axis, label):
        axis.annotate(label, xy=(0.1, 0.9), xycoords="axes fraction",
                    fontsize=60, ha='left')
    ...
    def plot_profile(self, axis, profile, annotation_gap, profile_scale_factor=1,
        ...
        if plot_error and ("Errors" in data.columns):
            yerr = data["Errors"]
            axis.bar(nts[mn-1:mx], values[mn-1:mx]*factor, align="center",
    ...
    def plot_annotation(self, axis, annotation, yvalue, mode):
        ...
        if a_type == "spans" or (a_type == "primers" and mode == "vbar"):
            ...
            sites = annotation[:]
            if mode == "track":
                axis.scatter(sites, [yvalue]*len(sites),
                           color=color, marker='*',
            ...
            if mode == "track":
                axis.plot(span, [yvalue, yvalue],
                        color=color, alpha=0.4, lw=11)
                axis.scatter(sites, [yvalue]*len(sites),
                            color=color, marker='o', ec="none", alpha=0.7,
                            s=11**2)
            elif mode == "vbar":
                axis.axvspan(span[0]-0.5, span[1]+0.5,
                            fc=color, ec="none", alpha=0.1)
                for site in sites:
                    axis.axvspan(site-0.5, site+0.5, fc=color, ec="none",
                               alpha=0.1)
        elif a_type == "primers":
            for start, end in annotation:
                if mode == "track":
                    if start < end:
                        ...
                        xs = [start, end, end+1]
                        ys = [yvalue, yvalue, yvalue-1]
                    axis.plot(xs, ys, color=color, alpha=0.7)
