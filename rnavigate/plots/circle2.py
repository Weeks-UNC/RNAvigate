...
class Circle(plots.Plot):
    ...
    def set_figure_size(
            ...
            height_ax_rel=1/pi/12, width_ax_rel=1/pi/12, width_ax_in=None,
    ...
    def plot_sequence(self, ax, profile, colors, apply_color_to):
        ...
        if apply_color_to == "background":
            bg_color, colormap = sequence.get_colors(colors, profile=profile)
            self.add_colorbar_args(colormap)
        if apply_color_to == "sequence":
            nt_color, colormap = sequence.get_colors(colors, profile=profile)
            self.add_colorbar_args(colormap)
